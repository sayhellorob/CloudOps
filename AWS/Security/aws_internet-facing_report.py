...
#############################
# End Recursive Analysis Functions
#############################
...
for region in regions:
    ...
    for lb in elb_client.describe_load_balancers().get("LoadBalancerDescriptions", []):
        if lb.get("Scheme") == "internet-facing":
            ...
            sg_ids, open_ports = get_open_ports_with_sg(security_groups, region)
            # For classic ELBs, listener ports might not be available in the same way.
            listener_ports = "N/A"
            all_data.append({
    ...
    # -------------------------
    # NEW SECTION: Collect Public ALBs (Application Load Balancers)
    # -------------------------
    try:
        ...
        for alb in alb_response.get("LoadBalancers", []):
            if alb.get("Scheme") == "internet-facing":
                ...
                sg_ids, open_ports = get_open_ports_with_sg(security_groups, region)
                listener_ports = (get_listener_ports(alb.get("LoadBalancerArn"), region)
                                  if alb.get("LoadBalancerArn") else "Managed by AWS")
                all_data.append({
                    "Component": "ALB",
                    "ID": alb.get("LoadBalancerName"),
                    "Public DNS": alb.get("DNSName"),
                    "Region": region,
                    "Security Groups": sg_ids,
                    "Open Ports": open_ports,
                    "Listener Ports": listener_ports,
        ...
        print(f"🚫 Error retrieving ALBs in {region}: {e}")
